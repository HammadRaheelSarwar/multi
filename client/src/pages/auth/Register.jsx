      ...
      >
        ...
        <div className="bg-white dark:bg-dark-800/80 border border-gray-200 dark:border-white/10 shadow-2xl rounded-3xl p-8 backdrop-blur-xl">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Register As</label>
              <div className="grid grid-cols-2 gap-4 mb-3">
                {/* Customer Option Card */}
                <div
                  onClick={() => setValue('role', 'customer')}
                  className={`relative flex flex-col items-center justify-center p-5 rounded-2xl border-2 cursor-pointer transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg select-none ${
                    selectedRole === 'customer'
                      ? 'border-blue-600 bg-blue-50/50 dark:bg-blue-950/20 text-blue-600 dark:text-blue-400 ring-4 ring-blue-500/10'
                      : 'border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-300 dark:hover:border-dark-700'
                  }`}
                >
                  <div className={`p-3 rounded-full mb-3 transition-colors ${selectedRole === 'customer' ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-gray-100 dark:bg-white/5'}`}>
                    <svg className={`w-7 h-7 ${selectedRole === 'customer' ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                  </div>
                  <span className="text-sm font-bold font-outfit">Customer</span>
                  <span className="text-[10px] text-center text-gray-400 dark:text-gray-500 mt-1 leading-tight">Find & book local services</span>
                  {selectedRole === 'customer' && (
                    <span className="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-white text-[10px]">✓</span>
                  )}
                </div>

                {/* Business Owner Option Card */}
                <div
                ...
                >
                  <div className={`p-3 rounded-full mb-3 transition-colors ${selectedRole === 'business_owner' ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-gray-100 dark:bg-white/5'}`}>
                    <svg className={`w-7 h-7 ${selectedRole === 'business_owner' ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                  </div>
                  <span className="text-sm font-bold font-outfit">Businessman</span>
                  <span className="text-[10px] text-center text-gray-400 dark:text-gray-500 mt-1 leading-tight">List business & offer services</span>
                  {selectedRole === 'business_owner' && (
                    <span className="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-white text-[10px]">✓</span>
                  )}
                </div>
              ...
              {/* Selected Role Alert/Feedback Panel */}
              <div className="rounded-xl p-3 bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 text-xs text-center font-medium text-gray-600 dark:text-gray-300 transition-all">
                Selected Role: <span className="font-bold text-blue-600 dark:text-blue-400 uppercase">{selectedRole === 'business_owner' ? 'Businessman (Service Provider)' : 'Customer'}</span>
              </div>
