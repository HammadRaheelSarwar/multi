...
const BusinessCard = ({ business }) => {
  ...
  return (
    <div
      onClick={handleClick}
        className={`group premium-card rounded-[20px] overflow-hidden transition-all duration-300 cursor-pointer hover:-translate-y-1 ${isFeatured ? 'border-t-2 border-t-[#e9c178]' : ''}`}
    >
      {/* Cover Image */}
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-semibold text-[#131c2a] dark:text-gray-100 font-outfit truncate text-base leading-tight">
            {name}
          </h3>
          {isVerified && (
            <ShieldCheck
        ...
        <div className="flex gap-2">
          {phone && (
            <a
              ...
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold text-[#131a33] dark:text-blue-300 border border-gray-300 dark:border-blue-900/50 rounded-lg hover:bg-gray-50 dark:hover:bg-blue-950/20 transition-colors"
            >
              <Phone size={12} />
              Call
            </a>
